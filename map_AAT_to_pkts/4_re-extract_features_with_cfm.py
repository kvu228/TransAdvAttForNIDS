...
fp_cfm = '/mmlab_students/storageStudents/nguyenvd/nids/CICFlowMeter/CICFlowMeter-4.0/bin/CICFlowMeter'
fp_input_dir = os.path.join(STORAGE_DIR, 'adv_pcap')
fp_output_dir = os.path.join(project_root_dir, 'output2')
...
lst_fp_allcsv = glob.glob(f'{fp_output_dir}/*')
lst_df = []
...
df.to_csv(os.path.join(fp_output_dir, 'final_aat.csv'), header=True, index=False)
